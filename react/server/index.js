...
const masterConfig = {
  user: 'sa',
  password: 'DmX08775416421',
  server: 'PLBIC14\\SQLEXPRESS',
  database: 'master',
  options: {
    encrypt: false,
...
app.get('/api/item-master', async (req, res) => {
  try {
    await sql.connect(config);
    const result = await sql.query(`
      SELECT ItemID, ItemName, SerialNo, MacNo, MinStock, CategoryID, registered_at, updated_at, pic
      FROM itemMaster
    `);
...
SELECT 
    ...
    (pb.Quantity - SUM(pb.ReceiveQty)) AS TotalDiffQty,  
