  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
        ...
        <Button variant="outlined" size="small" onClick={handleUpload} sx={{ mr: 2 }}>
          upload
        </Button>
      ...
      {mode === "ACTUAL" ? (
        <>
          ...
          {/* ====== ตาราง Raw เดิม ====== */}
          {/* <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
            <Typography sx={{ fontWeight: "bold", mb: 1 }}>Actual (Raw)</Typography>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: "bold" }}>ID</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>REV</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>WORKING / START</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>END</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>ACTUAL OUTPUT</TableCell>
              ...
              <TableBody>
                {actualRows.map((r, i) => (
                  <TableRow key={r.id ?? `${r.rev}-${r.startDate || r.workingDate}-${i}`}>
                    <TableCell>{r.id}</TableCell>
                    <TableCell>{r.rev}</TableCell>
                    <TableCell>{String(r.workingDate ?? r.startDate ?? "")}</TableCell>
                    <TableCell>{String(r.endDate ?? "")}</TableCell>
                    <TableCell>{r.actualOutput}</TableCell>
          ...
          {/* <Paper variant="outlined" sx={{ p: 2 }}>
            <Typography sx={{ fontWeight: "bold", mb: 1 }}>Plan (Approved)</Typography>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: "bold" }}>ID</TableCell>
                  ...
                  <TableCell sx={{ fontWeight: "bold" }}>IS MACHINE CONTINUE</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>PLAN TYPE</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>MACHINE NO</TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>BRG NO VALUE</TableCell>
                </TableRow>
              ...
              <TableBody>
                {planRows.map((r, i) => (
                  <TableRow key={r.id ?? `${r.rev}-${r.workingDate}-${i}`}>
                    <TableCell>{r.id}</TableCell>
                    <TableCell>{r.rev}</TableCell>
                    <TableCell>{String(r.workingDate ?? "")}</TableCell>
                    <TableCell>{r.planTarget}</TableCell>
                    <TableCell>{String(r.isMachineContinue)}</TableCell>
                    <TableCell>{r.planType ?? ""}</TableCell>
                    <TableCell>{r.machineNo ?? r.layout_row ?? ""}</TableCell>
                    <TableCell>{r.brgNoValue ?? ""}</TableCell>
                  </TableRow>
                ))}
        ...
        // ================= COMBINE =================
        <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
          {/* <Typography sx={{ fontWeight: "bold", mb: 1 }}>Combine (Plan vs Actual)</Typography>
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: "bold" }}>Machine No</TableCell>
            ...
            <TableBody>
              {combinedRows.map((r, i) => (
                <TableRow key={`${r.machineNo}|${r.brgNoValue}|${i}`}>
                  <TableCell>{r.machineNo}</TableCell>
                  <TableCell>{r.brgNoValue}</TableCell>
                  <TableCell>{r.startDate}</TableCell>
                  <TableCell>{r.endDate}</TableCell>
                  <TableCell align="right">{new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(r.planTotal)}</TableCell>
                  <TableCell align="right">{new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(r.actualTotal)}</TableCell>
                  <TableCell align="right">{new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(r.gap)}</TableCell>
          ...
          <Box sx={{ mt: 1, mb: 2 }}>
            <Button variant="outlined" size="small" onClick={downloadCombine}>
              download
            </Button>
          </Box> */}

          {/* ====== COMBINE แนวนอน (รายแถว: วันละ 2 บรรทัด Plan/Actual) ====== */}
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 1, flexWrap: "wrap" }}>
